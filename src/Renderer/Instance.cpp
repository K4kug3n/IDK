...
namespace Idk {
	Instance::Instance() :
		m_instance{ nullptr } {
		#define IDK_INSTANCE_FUNCTION(fun) fun = nullptr;
	...
	bool Instance::create(VkInstanceCreateInfo const& infos){
		VkResult result{ Loader::vkCreateInstance(&infos, nullptr, &m_instance) };
		if (result != VkResult::VK_SUCCESS) {
		...
		try {
			#define IDK_INSTANCE_EXT_FUNCTION_BEGIN(ext) if(isLoadedExtension(#ext)) {
			#define IDK_INSTANCE_EXT_FUNCTION_END() }
			#define IDK_INSTANCE_FUNCTION(fun) fun = reinterpret_cast<PFN_##fun>(loadInstanceFunction(#fun));
	...
	PFN_vkVoidFunction Instance::loadInstanceFunction(const char* name){
		PFN_vkVoidFunction fun{ Loader::getInstanceProcAddr(m_instance, name) };
		if (!fun) {
